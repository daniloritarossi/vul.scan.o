...
`POST /api/findings/tickets/refresh` (button **REFRESH TICKET STATUS**, writer roles) re-reads the state of every already-opened ticket the caller can see and stores it on the finding (`ticket_status`, `ticket_state`, `ticket_checked_at`). The state is moved by people in *their* tracker, so anything remembered here on its own would be a copy that ages in silence: it is read on demand rather than assumed. **Jira** answers the whole set in one `POST /rest/api/3/search/jql` call — the older `GET /rest/api/3/search` is gone, HTTP 410 — and **GitHub** costs one call per issue, which is the limit of that API. The row shows the provider's own label (`Done`, `In Progress`, `closed`) while the colour comes from the normalised category behind it (`todo` / `in_progress` / `done` / `unknown`), because a Jira workflow may name a status anything in any language. A GitHub issue closed as `not_planned` is reported closed but **not** done — closed is not the same as fixed. Scoped roles refresh only their own cone; audited as `finding.ticket_refresh`. **It updates the ticket, never the finding.** A ticket that turns `done` leaves its finding exactly where it was, still counted under OPEN in the KPIs — because "the ticket is closed" and "the vulnerability is fixed" are not the same statement: tickets get closed as *won't fix*, because a sprint ended, or by mistake, and a `not_planned` closure on GitHub is reported closed but explicitly not done. Closing a finding is a person's decision, taken in the STATUS column and recorded in the ledger with a name on it.

If a closed ticket does not close the finding, the finding needs somewhere for the work to
continue. The application **never reopens a ticket** on the tracker — it writes there once, at
creation, and the relationship is read-only from then on. Instead `POST /api/findings/{id}/ticket`
accepts `{"replace": true}` and opens a **second** ticket, but only where the current one can no
longer carry work: closed and completed while the finding is still open (typically a vulnerability
that came back in a later scan and was reopened automatically), closed **without a fix**
(`not_planned` on GitHub — a decision not to act, not a remediation), or opened on a tracker that is
no longer the configured one. It is refused while the current ticket is open or in progress (two
tickets on one vulnerability is duplicated work discovered downstream), when the finding is already
`fixed`/`accepted`, and when the ticket state has never been read from the tracker — opening one
blind could place it alongside a ticket that is still alive. The rule lives in
`app.ticket_replacement()` and is enforced server-side; `/api/findings` returns
`ticket_replaceable` per row so the UI offers only what the endpoint would accept.

The previous reference is **archived, not overwritten**: it moves into `ticket_history` (jsonb) with
its reason and date, and is exported in the `ticket_previous` column. The `n previous` chip in the ticket cell opens the **full history**: a timeline of every ticket the finding has had, each superseded by the one below it, with its state, the reason and the dates — the current ticket is the last entry rather than a separate thing, and every reference links straight to the tracker. `ticket_opened_at` records when the current ticket was raised; entries predating that column show *date not recorded* rather than a blank. A ticket closed as *won't fix*
records that somebody decided not to act, and when — overwriting it would erase exactly the fact an
audit asks about. Audited as `finding.ticket_create` carrying `replaces` and `reason`. Changing provider does not delete the tickets opened with the previous one, and those references are not queryable on the new tracker — `#42` means nothing to Jira, `SEC-101` nothing to GitHub. They are classified `foreign` (wrong tracker) as distinct from `unresolved` (right shape, not returned: deleted, moved, or out of the token's reach), reported separately rather than counted as checked, cost no provider call, and their rows are shown **frozen** — greyed, clock-stamped, the last state read and no longer updated.
